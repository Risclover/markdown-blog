      ...
      <div className="grid grid-cols-1 gap-1 grid-flow-row ">
        {postList.map((post) => (
          <div key={post.id} className="container py-2 bg-white mt-12">
            <h3 className="text-xl font-bold leading-8">{post.title}</h3>
            {post.subtitle !== "No subtitle given" && (
              <div className="text-slate-500 text-lg leading-8">
                {post.subtitle}
              </div>
            )}
            {post.description !== "No description given" && (
              <div className="mt-4 font-normal text-gray-900">
                {post.description}
              </div>
            )}
            <div className="mt-4 font-bold text-neutral-900">Read more</div>
          </div>
        ))}
      </div>
    </div>
